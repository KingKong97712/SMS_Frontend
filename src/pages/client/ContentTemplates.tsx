...
export default function ContentTemplates() {
  const [templates, setTemplates] = useState<Template[]>([
    ...
    },
    {
      ...
      content: 'Hi {{name}}! Get {{discount}}% off on your next purchase. Use code {{code}} before {{date}}.',
    ...
    },
    {
      ...
      content: 'Hello {{name}}, reminder for your appointment on {{date}} at {{time}}. Reply CONFIRM to confirm.',
  ...
  const handleSaveTemplate = () => {
    const content = editingTemplate ? editingTemplate.content : newTemplate.content;
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case 'promotion': return 'bg-gradient-to-r from-orange-500 to-red-500';
      case 'reminder': return 'bg-gradient-to-r from-blue-500 to-cyan-500';
          ...
          <div className="relative z-10">
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                ...
                <p className="text-violet-100 text-lg">Create and manage SMS message templates</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6">
                <div className="text-3xl font-bold">{templates.filter(t => t.category === 'welcome').length}</div>
                <div className="text-violet-100">Welcome</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6">
                <div className="text-3xl font-bold">{templates.filter(t => t.category === 'promotion').length}</div>
                <div className="text-violet-100">Promotional</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6">
                <div className="text-3xl font-bold">{templates.filter(t => t.category === 'reminder').length}</div>
              ...
              <CardHeader className="bg-gradient-to-r from-violet-500 to-purple-500 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-violet-100">Manage your message templates</CardDescription>
                  </div>
                  <Button
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  {filteredTemplates.map((template) => (
                    <div key={template.id} className="bg-gradient-to-r from-violet-50 to-purple-50 rounded-2xl p-6 border border-violet-200">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            <h3 className="text-xl font-bold text-violet-900">{template.name}</h3>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {template.placeholders.map((placeholder) => (
                              <Badge key={placeholder} variant="outline" className="bg-white text-violet-700 border-violet-300">
          ...
          <div className="space-y-6">
            {(isCreating || editingTemplate) && (
              <Card className="border-0 shadow-2xl bg-gradient-to-br from-white to-indigo-50 rounded-3xl overflow-hidden">
                ...
                <CardContent className="p-6 space-y-4">
                  ...
                  <div>
                    <label className="text-sm font-semibold text-gray-700 mb-2 block">Message Content</label>
                    <Textarea
                  ...
                  <Button
                    ...
                    disabled={
                      (editingTemplate && (!editingTemplate.name || !editingTemplate.content)) ||
                      (!editingTemplate && (!newTemplate.name || !newTemplate.content))
                    }
            ...
            <Card className="border-0 shadow-2xl bg-gradient-to-br from-white to-green-50 rounded-3xl overflow-hidden">
              <CardHeader className="bg-gradient-to-r from-green-500 to-emerald-500 text-white">
                <CardTitle className="text-xl">Available Placeholders</CardTitle>
                <CardDescription className="text-green-100">Click to insert into template</CardDescription>
              </CardHeader>
              <CardContent className="p-6">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-sm text-green-600">{placeholder.description}</div>
                        </div>
